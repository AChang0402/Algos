...
UndergroundSystem.prototype.checkIn = function(id, stationName, t) {
    this.inTransit.push([id, stationName, t])
};
...
UndergroundSystem.prototype.checkOut = function(id, stationName, t) {
    for (let i=0; i<this.inTransit.length; i++){
        if(this.inTransit[i][0]===id){
            this.complete.push([this.inTransit[i][1], stationName, t-this.inTransit[i][2]])
...
UndergroundSystem.prototype.getAverageTime = function(startStation, endStation) {
    ...
    for (let i=0; i<this.complete.length; i++){
        if(this.complete[i][0]==startStation && this.complete[i][1]==endStation){
            sum += this.complete[i][2]
            count ++
        }
    }
    return sum/count;
...
console.log(undergroundSystem.inTransit)
undergroundSystem.checkOut(45, "Waterloo", 15);  // Customer 45 "Leyton" -> "Waterloo" in 15-3 = 12
undergroundSystem.checkOut(27, "Waterloo", 20);  // Customer 27 "Leyton" -> "Waterloo" in 20-10 = 10
undergroundSystem.checkOut(32, "Cambridge", 22); // Customer 32 "Paradise" -> "Cambridge" in 22-8 = 14
console.log(undergroundSystem.inTransit)
console.log(undergroundSystem.complete)
